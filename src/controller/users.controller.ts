...
class UsersController {
  ...
  async login(ctx: CommonControllerCTX, next: CommonControllerNEXT) {
    ...
    try {
      ...
      if (res) {
        ...
        ctx.body = response(0, "登录成功", { result, token })
      } else {
        ctx.status = 400
        ctx.body = response(1, "登录失败，密码错误", data)
      }
    } catch (err) {
      ctx.body = response(1, "登录出错，请重试", { data, err })
    }
  }
  ...
  async register(ctx: CommonControllerCTX, next: CommonControllerNEXT) {
    ...
    try {
      ...
      ctx.body = response(0, "注册成功", result)
    } catch (err) {
      ctx.body = response(1, "注册出错，请重试", { data, err })
    }
  }
}
